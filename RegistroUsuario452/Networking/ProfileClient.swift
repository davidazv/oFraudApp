...
class ProfileClient {
    ...
    func getUserProfile(token: String) async throws -> UserProfileResponse {
        guard let url = URL(string: "http://10.48.246.68:3000/auth/profile") else {
            fatalError("Invalid URL" + "http://10.48.246.68:3000/auth/profile")
        }
    ...
    func updateUserProfile(token: String, name: String?, email: String?) async throws -> UserResponseDto {
        guard let url = URL(string: "http://10.48.246.68:3000/users") else {
